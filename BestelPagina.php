...
if(isset($_POST["BestelSubmit"])) { // if submit is pressed
    if ($_SESSION["inlogstatus"] == True) { // if logged in
        ...
        $result = mysqli_query($connection, $query);
        $row = mysqli_fetch_row($result);
        $sqlemail = $row[7]; // email belonging to customerID
        ...
        $query = "INSERT INTO webshopklant VALUES ((SELECT max(WebCustomerID)+1 FROM webshopklant w),'".$naam."','".$land."','".$woonplaats."','".$adres."','".$postcode."','".$tel."','".$email."',0,0)";
        $result = mysqli_query($connection, $query);
